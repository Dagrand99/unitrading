"""Orchestrator for contract-monitor-gov-uk. Daily 08:00 UTC."""
...
def main() -> int:
    ...
    print(f"[gov.uk] fetching last {days} day(s)...", flush=True)
    try:
        hits = fetch_recent_awards(wl, days_back=days)
    except Exception as e:
        print(f"FATAL: gov.uk fetch failed: {e}", file=sys.stderr)
        return 2
    print(f"[gov.uk] {len(hits)} matching award(s)", flush=True)
    process_hits(
        source_name="gov.uk",
        hits=hits,
